...
const GptSearchBar = () => {
  ...
  const HandleGpt = async ()=>{
    const gptQuery = "Act as a movie recommendation guy you want suggest movie  " + searchText.current.value + "only give name do not add any star just text and give comma separated values"
    ...
    dispatch(addMoviesGpt({moviesName : gptMovies , moviesResults : dataResults}))
  }
  return (
    <div className="pt-[10%] w-[80%] items-center">
      <form className="grid grid-cols-12" onSubmit={(e)=>e.preventDefault()}>
        ...
        />
        <button className=" text-white  m-2 bg-red-700 col-span-3 " onClick={HandleGpt}>Search</button>
